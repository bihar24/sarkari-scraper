...
// headings, "important links" label/link pairs, tables preceded by a title).
...
function scrapJobDetail(html, url) {
  ...
  // NOTE: this selector mirrors the site's deeply nested table layout. If a
  // scrape returns only the "Post Link" record, the markup has drifted and
  // this selector needs re-validating against the live page.
  var tableScope =
    "div.pageContent > table > tbody > tr > td > div > h2 > table > tbody > tr > td > div:nth-child(1) > table > tbody > tr > td:nth-child(1) > div > table > tbody > tr > td > div > table > tbody > tr > td > div > table > tbody > tr > td > div table";
  // Extract Table 1
  arr1 = $(tableScope).eq(0).find("tr").toArray();
  ...
  // Extract Table 2
  arr1 = $(tableScope).eq(1).find("tr").toArray();
  for (i = 0; i < arr1.length; i++) {
    if (isTable($, arr1[i])) {
  ...
  data = mergeKeyValue(data, "IMPORTANT LINKS");
