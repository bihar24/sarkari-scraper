...
// Usage: node scrap-paper-detail.js -u <url> [flags]
// Scrapes a single previous-year-question-paper page and prints the PDF links.
...
var HELP = [
  ...
  "",
  "Flags:",
  "  -u, --url <url>            Papers page URL (required). The host must be one of: " +
...
function resolveDomain(rawUrl) {
  ...
  if (constant.PAPER_DOMAIN_LIST.indexOf(host) === -1) {
    return {
      error:
        'Domain "' +
        host +
...
async function main() {
  ...
  if (sources.statusOf(resolved.domain, "papers") === "beta") {
    log.warn(
      '"' +
        resolved.domain +
  ...
  var paperDetail = sources.requireDetailParser(resolved.domain, "papers");
  ...
  try {
    ...
    data = paperDetail.scrapPaperDetail(response.data, pageUrl);
  } catch (error) {
  ...
  if (!Array.isArray(data) || data.length <= 1) {
    log.warn(
      "detail parser matched nothing on this page (only the " +
        '"Post Link" record was produced). The markup in scripts/' +
        resolved.domain +
        "/papers-detail.js may need updating."
    );
  } else {
